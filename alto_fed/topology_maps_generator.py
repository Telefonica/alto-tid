...
class TopologyCreator:
    ...
    def __init__(self):
        self.__speaker_bgp = ManageBGPSpeaker()
        self.__exabgp_process = self.__speaker_bgp.check_tcp_connection()
        #self.props = {}
    ...
    def load_topology(self, lsa, igp_metric, tp, pid):
        if lsa.get('ls-nlri-type') == 'bgpls-link':
            ...
            for i, j in zip(src, dst):
                #coste[i][j] = igp_metric
                tp.add_edge(i, j, weight=igp_metric)
        if lsa.get('ls-nlri-type') == 'bgpls-prefix-v4':
            ...
            for item in origin:
                if item not in tp.nodes():
                    tp.add_node(item)
                if 'prefixes' not in tp.nodes[item]:
                    tp.nodes[item]['prefixes'] = []
                tp.nodes[item]['prefixes'].append(prefix)
                if item not in pid.keys():
                    pid[item] = []
                #if 'prefixes' not in pid.[item]:
                #    self.topology.nodes[item]['prefixes'] = []
                pid[item].append(prefix)
        if lsa.get('ls-nlri-type') == "bgpls-node":
            ...
            for node_descriptor in node_descriptors:
                if node_descriptor not in tp.nodes():
                    tp.add_node(node_descriptor)
                #if node_descriptor not in coste.keys():
                #    coste[node_descriptor] = {}
                #    coste[node_descriptor][node_descriptor]= 0
        #print(tp.nodes())
        #print(tp.edges())
    ...
    def manage_bgp_speaker_updates(self):
        ...
        :return:
        """
        pids_to_load = deepcopy(self.pids)
        tp = self.topology.copy()
        #costes = deepcopy(self.costs)
        tiempo = ""
        asn = ""
        waiting = True
        while waiting:
            line = self.__exabgp_process.stdout.readline().strip()
            if b'decoded UPDATE' in line and b'json' in line:
                ...
                asn = decode_line['neighbor']['asn']['peer']
                tiempo = decode_line['time']
                if 'announce' in update_msg:
                        ...
                        if is_bgp_ls:
                            for next_hop_address, nlri in is_bgp_ls.items():
                                for prefix in nlri:
                                    ...
                                    node = ""
                                    r_node = ""
                                    for rid in prefix["local-node-descriptors"]:
                                        if "router-id" in rid.keys():
                                            node = rid["router-id"]
                                            continue
                                    for rid in prefix["remote-node-descriptors"]:
                                        if "router-id" in rid.keys():
                                            r_node = rid["router-id"]
                                            continue
                                    if node == "" or r_node == "":
                                        print("Cachis")
                                        continue
                                    if node not in costes.keys():
                                        costes[node] = []
                                    costes[node].append({"remote-node":r_node, "metric":igp_metric})'''
                                    self.load_topology(prefix, igp_metric, tp, pids_to_load)
                    ...
                    for route in update_msg['withdraw']['bgp-ls bgp-ls']:
                        ...
                        for field, values in route.items():
                            ...
                            if u != 0 and v != 0:
                                try:
                                    tp.remove_edge(self.split_router_ids(u), self.split_router_ids(v))
                                    #eliminar = -1
                                    #for i in range(costes[u].lenght()):
                                    #    if costes[u][i]["remote-node"] == v:
                                    #        eliminar = i
                                    #        continue
                                    #if eliminar != -1:
                                    #    costes[u].remove(i)
                                    #    eliminar = -1
                                    #for i in range(costes[v].lenght()):
                                    #    if costes[v][i]["remote-node"] == u:
                                    #        eliminar = i
                                    #        continue
                                    #if eliminar != -1:
                                    #    costes[v].remove(i)
...
if __name__ == '__main__':
    ...
    for i in range(20):
        t,a,p,c = topology_creator.manage_bgp_speaker_updates()
        print(t,a,p,p,c)
